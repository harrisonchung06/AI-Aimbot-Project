...
def pid(ix, epx, iy, epy, dx, dy, dt, kpx, kix, kdx, kpy, kiy, kdy):
    #print(f'vars{ix, epx, iy, epy, dx, dy, dt, kpx, kix, kdx, kpy, kiy, kdy}') Shows the hyperparameters of the control loop 
    dt = dt*3000
    if(ix > 320000):
        ix = 250000
    if(iy > 210000):
...
def detect(detect_feed, src, enemieslist):
    ...
    for i, row in bbox.iterrows():
        ...
        x0,y0,x1,y1 = x0+(int(left)), y0+(int(top)), x1+(int(left)), y1+(int(top)) #Cropping from offset 
...
while True:
    if not(0<=s and s<=1):
        s=0
        print("Off")
    a = win32api.GetKeyState(0x26)  # Press the up arrow key to start
    if a<0:
        s+=1
        print("On")
    b = win32api.GetKeyState(0x28) #Press down key to end 
    if b<0:
        s-=1
        print("Off")
    ...
    cv2.imshow("Detection Window", src)
    #cv2.imshow("Detection Window Processed", detect_feed)
